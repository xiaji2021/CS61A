...
class TutorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        """
        "*** YOUR CODE HERE ***"
        if len(opponent.deck.cards)  >= 3 and len(opponent.hand) >=3:
            for i in range(3):
                opponent.hand.pop()
...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        add_num = other_card.attack + other_card.defense
        cover_card = Card('card', add_num, add_num)
        for i in range(len(player.deck.cards)):
            player.deck.cards[i] = cover_card
            #这里注意一下究竟是引用是复制
        #print(opponent.deck.cards)
        tmp_cards = opponent.deck.cards[:]
        for rm_card in tmp_cards:
            if rm_card.attack == other_card.attack or rm_card.defense == other_card.defense:
                opponent.deck.cards.remove(rm_card)
                #这里注意一下是否真的移除
        #print(opponent.deck.cards)
